...
function on_create_move(cmd)
    ...
    if local_player:get_zombie_class() ~= 4 then return end
--if local_player:get_zombie_class() ~= zombie_class.SPITTER then return end
    local current_buttons = cmd:get_buttons()

    if input.is_key_down(keys.MOUSE1) then
        static_tick_count = static_tick_count + 1

        if static_tick_count % 2 == 0 then
            cmd:set_buttons(current_buttons & ~IN_ATTACK)
        else
            cmd:set_buttons(current_buttons | IN_ATTACK)
        end
    else
